...
Sub SeleccionarPlan()
	...
	wait 2
	While JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Alta Express (Para WALTHER").JavaStaticText("Mostrando 1-6 de 20 ofertas").Exist = true
		wait 1
	Wend
	wait 1
...
Sub AltaExpress()
	...
	ElseIf  ucase(Financiamiento) = "NO" and TipoAlta = "Alta solo SIM" Then
		JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Alta Express (Para WALTHER").JavaButton("Calcular").Click
		Dim c
		c = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Alta Express (Para WALTHER").JavaStaticText("0.00(st)").GetROProperty("text")
		while (c="0.00" or c="0,00")
			wait 1
			c = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Alta Express (Para WALTHER").JavaStaticText("0.00(st)").GetROProperty("text")
		wend
		wait 1
...
Sub NPC()
	...
	Call Carga()
	wait 2
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTree("Subproductos disponibles").Expand "#0;Móvil;SIM y Dispositivo;Tarjeta SIM"
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTree("Subproductos disponibles").Select "#0;Móvil;SIM y Dispositivo;Tarjeta SIM;Bonificación sobre precio de Simcard"
	wait 1
	...
	wait 5
	Dim filas2, l, valor
	filas2 =  JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:").GetROProperty("rows")
	For valor = filas2-1 To 0 Step -1
		JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:").SelectRow ("#"&valor)		
		l = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:").GetCellData("#"&valor, "#1")	    		
	   	l = Instr(1,l,"New attribute description")
		If l = "749" Then	
		      	valor = valor - 1
		      	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:").SelectRow ("#"&valor)
		      	JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() &"Bonificacion.png", True
				imagenToWord "Bonificación agregada",RutaEvidencias() &"Bonificacion.png"
		      	Exit for	    	    
...
Sub Delivery()
	...
	Wend
	wait 5
	if JavaWindow("Ejecutivo de interacción").JavaDialog("Mensaje").JavaStaticText("Dirección NO Valida, por").Exist = true Then
...
'RunAction "WIC_2", oneIteration
	...
	wait 3
	Dim g
	g = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Resumen de la orden (Orden").JavaCheckBox("El cliente firmó.").GetROProperty("enabled")
	If g = "1" Then
